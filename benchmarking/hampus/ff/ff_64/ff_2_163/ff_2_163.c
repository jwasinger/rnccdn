...
ff_element ff_zero = { 0,0,0,0,0,0 };
ff_element ff_one = { 1,0,0,0,0,0 };
...
void reduce ( ff_block* a ) {
  ...
  t = a[BLOCKS-1] & (0xFFFFFFFF-7);
  a[BLOCKS-1-4] ^= t>>29 ^ t>>28;
  a[BLOCKS-1-5] ^= t>>3 ^ t ^ t<<3 ^ t<<4;
  a[5] &= 7;
}
...
void ff_mul_lrcomb_w4 ( ff_element a, ff_element b, ff_element c ) {
  ...
  if ( base[ 1*BLOCKS+BLOCKS-1 ] & 4 ) {
    base[2*BLOCKS+BLOCKS-1] &= 7;
    base[2*BLOCKS+0] ^= 201;
  }
  ...
  if ( base[ 2*BLOCKS+BLOCKS-1 ] & 4 ) {
    base[4*BLOCKS+BLOCKS-1] &= 7;
    base[4*BLOCKS+0] ^= 201;
  }
  ...
  if ( base[ 4*BLOCKS+BLOCKS-1 ] & 4 ) {
    base[8*BLOCKS+BLOCKS-1] &= 7;
    base[8*BLOCKS+0] ^= 201;
  }
...
void ff_read ( ff_element a ) {
  scanf("%x %x %x %x %x %x", a->data+5, a->data+4, a->data+3, a->data+2, a->data+1, a->data+0);

  assert( !(a->data[BLOCKS-1] & !7) );
  a->data[BLOCKS-1] &= 7;
}

void ff_sread ( const char * s, ff_element a ) {
  sscanf(s, "%x %x %x %x %x %x", a->data+5, a->data+4, a->data+3, a->data+2, a->data+1, a->data+0);
...
void ff_print ( ff_element a ) {
  printf("[%x %x %x %x %x %x]", a->data[5], a->data[4], a->data[3], a->data[2], a->data[1], a->data[0]);
